...
def test_observability_client_caching() -> None:
    ...
    with (
        patch.dict(
            os.environ, {"GOOGLE_CLOUD_PROJECT": "p1", "OBSERVABILITY_ENABLED": "1"}
        ),
        patch("google.cloud.monitoring_v3.MetricServiceClient") as MockClient,
    ):
        obs = ObservabilityService()
        obs.enabled = True
...
def test_observability_agent_run() -> None:
    ...
    obs = ObservabilityService()
    with patch.object(obs, "schedule_metric") as mock_sched:
        obs.schedule_agent_run(
            "test_agent", 50.0, status="success", fallback=True, output_size_bytes=100
        )
        calls = [c[0][0] for c in mock_sched.call_args_list]
