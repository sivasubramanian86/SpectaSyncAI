...
async def test_get_pool_singleton():
    with patch("asyncpg.create_pool", new_callable=AsyncMock) as mock_create_pool, \
         patch("pgvector.asyncpg.register_vector", new_callable=AsyncMock) as mock_register, \
...
async def test_embed_mock():
    with patch("vertexai.init"), \
         patch("vertexai.language_models.TextEmbeddingModel.from_pretrained") as mock_from_pre:
...
async def test_alloydb_memory_real_mode_success():
    with patch.dict(os.environ, {"DATABASE_URL": "postgresql://user:pass@host/db"}), \
        ...
        mock_conn.fetch.return_value = [
            {"incident_id": "I1", "description_text": "D1", "interventions_json": "[]", "failure_modes": ["M1"], "similarity_score": 0.9}
        ]
        mock_pool.acquire.return_value.__aenter__.return_value = mock_conn
        mock_get_pool.return_value = mock_pool
