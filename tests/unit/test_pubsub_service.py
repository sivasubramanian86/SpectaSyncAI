...
async def test_pubsub_service_broadcast_enabled():
    ...
    with patch('google.cloud.pubsub_v1.PublisherClient') as mock_client_class:
        ...
        with patch.dict('os.environ', {'K_SERVICE': 'test-service', 'GOOGLE_CLOUD_PROJECT': 'test-project'}):
            service = PubSubService()
...
async def test_pubsub_service_initialization_error():
    ...
    with patch('google.cloud.pubsub_v1.PublisherClient', side_effect=Exception("Auth error")):
        with patch.dict('os.environ', {'K_SERVICE': 'test-service'}):
            service = PubSubService()
            assert service.enabled is False
