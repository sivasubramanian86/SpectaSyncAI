...
@router.get(
    "/telemetry/{location_id}",
    summary="Get latest telemetry metadata for a zone"
)
async def get_telemetry_meta(location_id: str) -> dict:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Telemetry pipeline metadata failure."
        )
...
async def ingest_telemetry(payload: TelemetryPayload) -> OrchestratorResponse:
    ...
    if payload.image_b64:
        ...
        try:
            density_report = await run_vision_analysis(
                payload.location_id,
                image_bytes
            )
        except Exception as exc:
